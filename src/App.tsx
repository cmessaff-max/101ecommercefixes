...
export default function App() {
  ...
  const checkEmailAccess = useQuery(api.emails.checkEmailAccess, 
    email ? { email } : "skip"
  );
  ...
  const handleBottomEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await subscribeEmail({ email: bottomEmail });
      
      if (result.success) {
        // Show success message
        toast.success("Email saved! Opening Google Sheet with all 101 fixes...", {
          duration: 3000,
        });
        
        // Open Google Sheet in new tab after a brief delay
        setTimeout(() => {
          window.open('https://docs.google.com/spreadsheets/d/1BaeOJeP5oIpbumgh5VN4LJZAhjuByTSssxbMlHuyHrI/edit?gid=0', '_blank');
        }, 1000);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#050610', fontFamily: 'Poppins, sans-serif' }}>
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-sm border-b shadow-sm" style={{ backgroundColor: 'rgba(5, 6, 16, 0.95)', borderColor: '#12141C' }}>
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="font-bold text-2xl tracking-wider" style={{ fontFamily: 'Orbitron, monospace', letterSpacing: '0.1em' }}>
      ...
      {/* Hero Section */}
      <section className="hero py-20" style={{ background: 'linear-gradient(135deg, #050610 0%, #0F1226 50%, #02011A 100%)' }}>
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="text-5xl md:text-6xl font-medium text-white mb-8 leading-tight" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 500 }}>
            Steal <span className="bg-gradient-to-r from-pink-400 via-purple-400 to-pink-300 bg-clip-text text-transparent">101 <span className="text-6xl md:text-7xl">Proven</span> Fixes</span> <span className="text-6xl md:text-7xl">Ecommerce</span> Brands Use to <span className="bg-gradient-to-r from-pink-400 via-purple-400 to-pink-300 bg-clip-text text-transparent">Stop Wasting <span className="text-6xl md:text-7xl">Ad Money</span> and Skyrocket <span className="text-6xl md:text-7xl">Sales</span></span>
          </h1>
          <p className="subtitle text-lg md:text-xl text-gray-300 mb-8 max-w-3xl mx-auto leading-snug" style={{ letterSpacing: '0.025em' }}>
            Get immediate access to the 101 Ecommerce Fixes to Boost Conversions. Quick wins and deep strategies top DTC operators use to turn paid ads and email sequences into revenue machines.
          </p>
      ...
      <section id="easy" className="py-16" style={{ backgroundColor: '#0F1226' }}>
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="accordion rounded-xl p-8 border" style={{ background: 'linear-gradient(135deg, #12141C, #0E1125)', borderColor: '#8A2BE2' }}>
            <button 
              onClick={() => toggleSection('easy')}
              className="toggle w-full flex justify-between items-center text-left"
            >
              <span className="text-xl font-semibold text-white">View Easy Fixes (1-34) - 1-2 days</span>
              <span className="text-2xl bg-gradient-to-r from-pink-400 to-purple-400 bg-clip-text text-transparent">{expandedSection === 'easy' ? '−' : '+'}</span>
            </button>
      ...
      <section id="medium" className="py-16" style={{ backgroundColor: '#02011A' }}>
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="accordion rounded-xl p-8 border" style={{ background: 'linear-gradient(135deg, #12141C, #0F1227)', borderColor: '#A020F0' }}>
            <button 
              onClick={() => toggleSection('medium')}
              className="toggle w-full flex justify-between items-center text-left"
            >
              <span className="text-xl font-semibold text-white">View Medium Fixes (35-67) - 1-2 weeks</span>
              <span className="text-2xl bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">{expandedSection === 'medium' ? '−' : '+'}</span>
            </button>
      ...
      <section id="hard" className="py-16" style={{ backgroundColor: '#0F1226' }}>
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="accordion rounded-xl p-8 border" style={{ background: 'linear-gradient(135deg, #12141C, #0E1125)', borderColor: '#FF2BA4' }}>
            <button 
              onClick={() => toggleSection('hard')}
              className="toggle w-full flex justify-between items-center text-left"
            >
              <span className="text-xl font-semibold text-white">View Hard Fixes (68-101) - 1-3 months</span>
              <span className="text-2xl bg-gradient-to-r from-pink-400 via-purple-400 to-pink-300 bg-clip-text text-transparent">{expandedSection === 'hard' ? '−' : '+'}</span>
            </button>
      ...
      <section id="download" className="py-20" style={{ background: 'linear-gradient(135deg, #02011A 0%, #0F1226 50%, #050610 100%)' }}>
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <form onSubmit={handleBottomEmailSubmit} className="max-w-lg mx-auto">
            <div className="flex flex-col sm:flex-row gap-4 items-stretch">
              <div className="flex-1">
                <label htmlFor="bottom-email" className="sr-only">Email address</label>
                <input
                  id="bottom-email"
                  type="email"
                  value={bottomEmail}
          ...
          <p className="text-sm text-gray-400 mt-6">
            No spam. Direct access to Google Sheet with all fixes.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="mb-6 md:mb-0">
              <div className="font-bold text-xl mb-2" style={{ fontFamily: 'Orbitron, monospace', letterSpacing: '0.1em' }}>
            ...
            <div className="flex items-center space-x-6">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold" style={{ background: 'linear-gradient(135deg, #FF007F, #A020F0)' }}>
                  CM
                </div>
                <div>
